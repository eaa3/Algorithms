...
int main()
{
    ...
    while( scanf("%d %d",&speed,&size) == 2 )
    {
        ...
        router.reinit();
		router.speed = speed;
		router.bufferSize = size;
		router.currentBufferSize = size;
        ...
        do
        {
            ...
            if(router.bufferQueue.hasNode())
            {
                ...
                {
                    processing = true;
					router.bufferQueue.root->packet.beginProcessTime = router.currentNanoseconds;
					router.bufferQueue.root->packet.timeOut = router.currentNanoseconds - router.bufferQueue.root->packet.arrivalTime  ;
                    
                }

                if( processing && router.currentNanoseconds == (router.bufferQueue.root->packet.beginProcessTime + router.bufferQueue.root->packet.processTime ) )
                {
                    bufPacket = router.getNextProcessed();
                    tempos += bufPacket.processTime;
					printf("%d: %s foi processado em %dns e %dns esperando na fila\n",router.currentNanoseconds,bufPacket.name,bufPacket.processTime,bufPacket.timeOut);
                    processing = false;


					if(router.bufferQueue.root && router.currentNanoseconds >= router.bufferQueue.root->packet.arrivalTime && !processing)
					{
						processing = true;
						router.bufferQueue.root->packet.beginProcessTime = router.currentNanoseconds;
						router.bufferQueue.root->packet.timeOut = router.currentNanoseconds - router.bufferQueue.root->packet.arrivalTime  ;
